...
type: "Knowledge"
title: "レイヤーを跨ぐデータの形（DTO等）の設計原則を対象とする概念：architecture-cross-layer-data-shape"
...
# レイヤーを跨ぐデータの形（DTO等）の設計原則を対象とする概念：architecture-cross-layer-data-shape
...
- DTOは「その層の外に出るための専用の入れ物」であり、必要なデータだけをコピーし、ふるまい（メソッド）を保持しない単純なデータ構造にする。
- 層をまたぐたびに変換処理（マッピング）が必要になるが、この変換コストは、レイヤー境界の独立性を維持するための対価として正当なものである。
...
| 外側から受け取ったDTOのフィールドを直接ドメインオブジェクトに代入する | 集約のメソッドを経由しない状態変更となり、不変条件のカプセル化が成立しなくなる |
...
| 関連概念 | 関係 |
|---|---|
| architecture-dependency-direction | 越境時のデータ変換は依存方向の原則を実務レベルで遵守するための具体的な手段 |
