...
- architecture-evidence-based-scope.mdを判断根拠に使う場合は、先回りが背負う2つのコスト（選択権の喪失と、価値実現までの時間差）が対象で何を指すかを回答内で示す。実例の件数で着手の可否を分類せず、可否の裁定も行わない ── 提示するのは意思決定者への分析結果である（template-judgment-tech-lead.mdの前置きセクション参照）
