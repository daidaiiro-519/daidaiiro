...
description: "進行中の作業の現在地や、外部システムへのポインタなど、**次のセッションが続きから始められるようにするためだけ**の一時的な事実を記録するときに使う。記録するのは project と reference の2種だけで、**恒久の決まり（次から遵守させる規則）は記録しない** ── それは、それが適用される Skill のガードレールか knowledge が持つ。"
---
...
- **恒久の決まりを記録しない**。記憶は一時的なものだけを置く場所であり、ここへ置くと、遵守させたい場所（Skill ・ knowledge）に無いまま増える
