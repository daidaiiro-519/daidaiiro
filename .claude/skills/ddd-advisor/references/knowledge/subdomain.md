...
書籍固有の実例（特定業界の企業例）は著作権への配慮から教材用の架空ドメイン（物流プラットフォーム・オンライン学習サービス・社内の備品貸出）へ置き換えている。ただし図が示す関係の構造は原文のまま保っている——置き換えて構造まで改変すると、示していたことが消えるため。
...
| どこの話か | 図に載せきれないこと |
|---|---|
| 誰かがお金を払ってくれる業務か | 収益が直接発生するかではなく、その業務が他社との違いを生んで対価につながるかを問う。社内向けの業務でも、それが競争力の源なら「YES」になる。 |
| 外部サービスの方が簡単で安いか | 現時点の費用の比較ではない。その業務を他社と同じやり方にしてしまってよいかを問う。安く済んでも差別化を放棄することになるなら「NO」である。 |
| 補完の業務領域 | 2本の線が入るが、由来が違う。上（対価につながるが外部で足りる）は外部へ寄せる判断、下（対価につながらず既製品も無い）は自前で作るしかない判断で、同じ補完でも扱いが変わる。 |
| 中核の業務領域／一般的な業務領域／補完の業務領域 | 業務領域ごとの分類であって、システムやチームの分類ではない。1つのシステムの中に3つとも入りうる。 |
...
| どこの話か | 図に載せきれないこと |
|---|---|
| 配送サポート窓口（一般？） | 疑問符は、細分化する前の暫定の見立てであることを表す。細分化した結果、この見立ては誤りだったと分かる。 |
| 配送サポート窓口（一般？）から案件の割り振り ── 中核 | 線は1本しか描いていないが、細分化で出てくるのは囲みの中の4つ全部である。1本にしたのは細分化という操作を示すためで、他の3つが無関係という意味ではない。 |
| 配送サポート窓口の細分化 | 4つが同じ階層に並んでいるが、分類は3種類に分かれる。囲みは、中身が同じ分類であることを示していない。 |
...
| どこの話か | 図に載せきれないこと |
|---|---|
| 配送手配 | 境界を先に引いてユースケースを入れたのではない。同じアクターと同じ外部システムを共有するユースケースが集まった結果として、境界が見えている。順序が逆になるとこの図の主張が消える。 |
| 到着を見込む | 囲みの中にありながら、他の4つと違う外部システムへつながる。これはこの1つだけが別の業務領域に属しうるという手がかりであり、囲みが正しいと確定させる図ではない。 |
| 荷主 | アクターであって業務領域の一部ではない。囲みの外にあるのはそのためで、業務領域に属していないという意味ではない。 |
...
一般に分類された社内の備品貸出システムを、在庫の照会・貸出の申請・返却の記録・期限の通知へ細分化しても、4つとも一般のままになる。分割の前後で分類が変化しないので、この細分化は設計判断を何も変えておらず不要。
...
| どこの話か | 図に載せきれないこと |
|---|---|
| 備品貸出システム ── 一般から4つへの「細分化」の線 | 細分化そのものが誤りだと言っているのではない。細分化した結果、分類が1つも変わらなかったことが不要の根拠である。 |
| 在庫の照会 ── 一般／貸出の申請 ── 一般／返却の記録 ── 一般／期限の通知 ── 一般 | 4つとも分類を書き添えてあるのは、分割の前後で分類が同じであることを見せるためである。分類が一致していること自体が、この図の結論にあたる。 |
...
中核の業務領域のソフトウェア開発を外部委託すると、短期的には安く見えるが長期的には会社の存続に関わる問題を招く。変更がやりにくく危険なコードベースになり、事業目標の達成を技術面で支援できなくなる。
...
分割しても設計判断が変化しないなら、その分割は必要ない。「分割した後のカテゴリーがすべて分割前と同じ」なら意味がない細分化。
