...
全面的に書き換えるより、小さな変更を段階的に加えるほうが安全である。始めどころは値オブジェクトの候補を見つけるところ——状態が変わらないオブジェクトを導入するだけでも、複雑さはかなり減る。そこから対象の業務ロジックを集め、ひと塊の変更の境界を詳しく調べる——技術の視点ではなく業務の視点で。業務の視点でその必要性を徹底的に分析して、はじめて集約の境界を設計できる。
...
| この並び全体 | 全面的に書き換えるのではなく、小さな変更を段階的に加える。始めどころは値オブジェクトの候補を見つけるところで、状態が変わらないオブジェクトを導入するだけでも複雑さはかなり減る。 |
