...
id: "domain-model"
type: "Knowledge"
title: "ドメインモデルの定義を対象とする概念：domain-model"
...
# ドメインモデルの定義を対象とする概念：domain-model
...
業務で処理する値を表現する部品。欄の値の組み合わせで識別し、明示的な識別子を保持しない。不変として実装し、変更するときは新しいインスタンスを返す。等価判定は識別子ではなく値で行う。とくに重要なのは金額の表現で、基本データ型で表現すると丸めや端数の処理に危険な不具合が紛れ込みやすい。
...
| どこの話か | 図に載せきれないこと |
|---|---|
| ひとつの集約 | 階層構造があれば集約になる、ではない。業務ロジックが3つすべてを同時に必要とする場合に限って、この囲みが引ける。 |
| 配送の依頼から連絡記録／連絡記録から添付（1対多） | データベースの表の関係ではなく、業務上の持ち物の関係を表す。表がどう分かれるかは、この図からは決まらない。 |
...
同じ集約の一部として一貫性を維持するものだけを内側に持ち、それ以外は識別子で指す。識別子で指すことが、「これは境界の外側だ」という表明になる。
...
| どこの話か | 図に載せきれないこと |
|---|---|
| アプリケーション層から配送の依頼（ルート） | ルートを通るという制約そのものを表す。アプリケーション層から連絡記録へ向かう線が無いのは、描き忘れではなく禁じられているためである。 |
| 配送の依頼（ルート） | 階層の一番上という意味ではなく、外から呼べる唯一の入口という意味である。 |
...
集約や値オブジェクトでは表現しにくい業務ロジック、または複数の集約にまたがる業務ロジックを記述するための部品。業務ロジックだけを記述し、自分自身の状態は保持しない。多くの場合、さまざまな部品の呼び出しを統合して計算や分析を行う。

業務サービスが、複数の集約から材料を集めて計算だけを行う形を示す

自分自身の状態は保持せず、方針と勤務予定という別々の集約から値を読み、締切を計算して返すだけ。どの集約の状態も変えない。
...
業務サービスは、マイクロサービスやサービス指向アーキテクチャの「サービス」という用語とは無関係である。業務ロジックの置き場所として使う、状態を保持しないオブジェクトを指す。

#### 抜け道ではない

1集約=1トランザクションの原則は変化しない。業務サービスは複数の集約を参照する計算を書くのに役立つが、複数の集約を単一のトランザクションで変更するための抜け道ではない。

### 複雑さの扱い方

集約と値オブジェクトの背景にある考え方は、不変条件をカプセル化して複雑さを小さくすること。システムの複雑さは自由度——独立して変化できる状態の個数——で決まる。不変条件を加えると制約が増え、自由度が下がり、振る舞いを制御・予測するのが簡単になる。値オブジェクトは値に関する規則を境界の内側に集め、集約は状態を変えられる経路を自分のメソッドだけに限定する。
...
識別子を持つのはエンティティと集約。状態が変わるのもその2つ。値オブジェクトと業務サービスはどちらも状態を保持しないが、前者は値そのもの、後者は計算の置き場所である。
...
| 業務サービス | なし | なし（状態を保持しない） | 複数の集約にまたがる計算ロジック |
...
| どこの話か | 図に載せきれないこと |
|---|---|
| 業務ロジックは複雑かの「はい（中核）」 | 複雑さと中核であることを同じものとして扱っている。厳密には別の判断だが、この図では中核であることを複雑さの目安として使っている。 |
| エンティティまたは集約 | 2つを1つの終点にまとめてあるが、エンティティは単独では実装しない。実際にはここから必ず集約へ進む。 |
| 手続きとして書く（別の実装方法） | この図の外へ出る終点である。ドメインモデルの部品を選ぶ話ではなくなる。 |
...
金額・電話番号・連絡先などを素の文字列や数値で表現すると、関連する業務ロジックがあちこちに散らばる。値オブジェクトで表現して内側に集める。
