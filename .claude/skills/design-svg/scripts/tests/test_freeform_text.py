...
class Test文字と絵記号:
    ...
    def test_揃え方を変えても外接矩形は動かない(self, align):
        style = _style()
    ...
    def test_知らない絵の名前は例外にする(self):
        style = _style()
        with pytest.raises(ValueError):
            render_component("icon", {"name": "無い絵"}, style)

    def test_絵は台帳から引く(self):
        """**一覧を2か所に持たない。**"""
        assert "doc" in _names() and "spark" in _names()

    def test_並べても幾何は破綻しない(self):
