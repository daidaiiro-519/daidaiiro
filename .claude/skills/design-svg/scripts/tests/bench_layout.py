...
1. 交差の数   ── 辺どうしが何度すれ違うか。並びの層(order)の出来を映す。
2. 辺の長さ   ── 通る点列の総延長。層の配分(rank)の出来を映す。
...
# 自分の親（Skill 直下）を import 経路へ追加する ── パッケージとして入れずに直接走らせるため
...
def nudge(nodes, edges) -> dict:
    ...
    実務で意味を持つ安定性はこちら。図を少し直すたびに全体が組み替わると、
...
def uneven():
    ...
    他の4案件はどれも層が一意に決まってしまい、層の配分の出来を測れない。
...
CASES = {"枝の多い木": tree(), "多層をまたぐ辺": skipping(),
