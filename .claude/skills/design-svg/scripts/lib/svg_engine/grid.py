...
層状配置は辺から層を決める。だが「対応」は辺を持たない ── 縦横の交点に何が来るかを
座標そのものが指定する。辺から決める戦略へ渡すと、座標が捨てられて1層に並ぶ。
...
def layout_grid(node_sizes: dict[str, tuple[float, float]],
    ...
    Args:
        ...
        at: 節点idごとの (列のキー, 行のキー)。渡されない節点は格子に載らない。
        cols / rows: 列・行の並び。渡さなければキーを昇順に整列した順になる。
            見出しを端の列・行へ置きたいときは、そのキーを先頭に入れて渡す
            ── 整列に頼ると、見出しが中ほどへ紛れ込む。
