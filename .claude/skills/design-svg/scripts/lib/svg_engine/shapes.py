...
囲みのラベルは、既に置かれた節点をまたぐので、置いた後にしか描けない ── だから
...
def arrow_head(tip: tuple[float, float], angle: float, style: Style,
               color: str | None = None, shape: str = "solid") -> str:
    """矢じりを1つ描く。向きは角度で受け取る。
    ...
    Args:
        ...
        color: 塗る色。省略すると線の色。
        shape: "solid"（塗った三角）か "open"（線だけの山形）。
            **形の選択であって、大きさとは別に持つ** ── 細い線の図に
            塗った三角を置くと、そこだけ重く見える。
    ...
    ink = color or style.text("color.line", style.text("color.ink-faint"))
    if shape == "open":
        return (f'<path d="M{hx1:.1f},{hy1:.1f} L{x2:.1f},{y2:.1f} L{hx2:.1f},{hy2:.1f}" '
                f'fill="none" stroke="{ink}" stroke-width="{style.num("size.stroke-width")}" '
                f'stroke-linecap="round" stroke-linejoin="round"/>')
    return (f'<polygon points="{x2:.1f},{y2:.1f} {hx1:.1f},{hy1:.1f} '
            f'{hx2:.1f},{hy2:.1f}" fill="{ink}"/>')
...
@component("panel")
def panel(props: dict, style: Style) -> OwnOrigin:
    """素の面 ── 塗りと枠だけを持つ矩形。

    **囲み（frame）とは別である** ── 囲みは「ここは領域の内側」を示す破線の注記で、
    面は意匠そのものである。同じ部品で兼ねると、どちらの意味で置いたかが読めない。

    props: width, height（必須）／radius（既定 size.box-radius）／
           fill（濃さの呼び名、または "none"。既定 "fill"）／
           stroke（同上。既定 "line"）
    """
    w, h = props["width"], props["height"]
    r = props.get("radius", style.num("size.box-radius"))

    def tone(name: str, fallback: str) -> str:
        if name == "none":
            return "none"
        return style.text(TONES[name], style.text(fallback))

    fill = tone(props.get("fill", "fill"), "color.box-fill")
    stroke = tone(props.get("stroke", "line"), "color.line")
    sw = style.num("size.stroke-width")
    svg = (f'<rect x="0" y="0" width="{w:.1f}" height="{h:.1f}" rx="{r:.1f}" '
...
def dot(props: dict, style: Style) -> OwnOrigin:
    ...
    props: radius（既定 size.dot-radius）／tone（濃さの呼び名。既定は本文の色）／
           stroke（枠の濃さ。省くと枠を描かない）
    """
    r = props.get("radius", style.num("size.dot-radius"))
    tone = props.get("tone")
    fill = (style.text(TONES[tone]) if tone
            else style.text("color.text", style.text("color.ink")))
    edge = props.get("stroke")
    line = (f' stroke="{style.text(TONES[edge])}" '
            f'stroke-width="{style.num("size.stroke-width")}"' if edge else "")
    svg = f'<circle cx="{r:.1f}" cy="{r:.1f}" r="{r:.1f}" fill="{fill}"{line}/>'
...
def frame_label(props: dict, style: Style) -> Absolute:
    ...
    ラベルは不透明な帯を持つので、線の上に載れば線を断って読める。避けられる
    場所があるなら避けるが、無いときは載る ── どちらにせよ線は曲げない。

    props: x（ラベルの左端の絶対座標）／y（枠線の上辺の絶対座標）／label
...
def frame(props: dict, style: Style) -> Absolute:
    ...
    ラベルは描かない ── ラベルは線より後に描く必要があり、置き場所も線を避けて決まる
