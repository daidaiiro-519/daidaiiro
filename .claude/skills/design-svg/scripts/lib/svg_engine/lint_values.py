...
なく、構造・数学・仕様が決めているか、あるいは既に名前を持っている。
    ...
    - 数の大小ではなく個数を検査する比較（`len(points) >= 3` のような構造の条件）
    - 対応表そのもの（キーから値を参照する辞書。数はその表の中身であって、式の中に
      隠れた選択ではない）
    - 数値誤差の許容値
...
使い方:
    python3 scripts/cli.py lint                  # 一覧を出す
...
class _Finder(ast.NodeVisitor):
    ...
    def visit_Compare(self, node: ast.Compare) -> None:
        """個数を検査する比較は構造の条件。`len(points) >= 3` の 3 は選んだ値ではない。"""
    ...
    def visit_Dict(self, node: ast.Dict) -> None:
        ...
        キーがすべて定数の辞書だけを表とみなす ── その場で組み立てる辞書は除く。
        """
