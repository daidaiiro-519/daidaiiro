...
DEFAULT_THEME: dict[str, TokenValue] = {
    ...
    # 強調の面に載せる文字。**地と同じ層に置く** ── 面だけ替えると文字が消える。
    "color.accent-fg": "#F5F7F9",
    # 自由な形（path・boolean）の塗りと線。**テーマが持つ** ── 部品の中で
    # 既定値へ縮退させると、その場の上書きが届かない（実測で届かなかった）。
    "color.shape-fill": "color.accent",
    "color.shape-stroke": "color.accent",
    "color.warn": "#9A4A21",
...
# 色の濃さの呼び名 ── 部品の props で受ける名前から、色のトークンへ。
# **直値を渡させない** ── 渡させると、テーマを替えたときにそこだけ取り残される。
TONES: dict[str, str] = {
    "ink": "color.ink",
    "soft": "color.ink-soft",
...
# 無いトークンは無制限（色・文字列トークンや、破綻しても致命的でない値）。
