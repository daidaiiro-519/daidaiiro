...
利用側（変換器を書く人）は、ここだけを参照すれば済む。どの部品があり、それぞれが
...
抽出したもの**である。部品を直せば目録も直る。直し忘れは起こらない。

ずれが起こりうるのは見本（EXAMPLES）だけなので、そこは試験で縛る ──
見本が目録に無いキーを渡していたら失敗する。
...
繋がっている証拠でもある。部品を足したらここへ1行足す ── 足し忘れは試験で検出される。
"""
...
def parts() -> dict[str, dict]:
    ...
    置き方と名前の申告は、実際に見本で描いて確認した結果を載せる ── 部品が
    返すものなので、呼ばずには分からない。
    """
    out: dict[str, dict] = {}
