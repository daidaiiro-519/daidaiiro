...
「障害物＝箱」という同じ前提を共有していたため、線が文字を貫く破綻を
どちらも見逃していた）。検査は独立に書く。
"""
...
# ── 描く側と検査側で粗さが違うと、片方だけが見つける破綻ができる。
SAMPLE_DIVISOR = 4
...
def densify(pts: list[Point], step: float) -> list[Point]:
    ...
    頂点だけを標本にすると、両端が箱の外にある直線が箱を貫いていても分からない。
...
def _arc_points(p0: Point, rx: float, ry: float, rot: float,
    ...
    点と点の間が開き、その間の向きが輪郭から欠落する（輪の部品で実測：
...
def sample_ink(fragment: str, step: float) -> list[tuple[Point, float]]:
    ...
    Args:
        ...
        step: 刻む間隔。角だけを収集すると、辺の途中が輪郭から欠落して、
            形が実物より膨らむ。
