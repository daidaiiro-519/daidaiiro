...
**1から4は、枚をどう作るかである。5から8は、作ったものをどう確認するかである。**
...
**日本語での現れ**——描画した画像を、作った本人とは別の目で確認する。
文脈を保持しない読み手に、成果物だけを渡して確認するのと同じ形である。

### 7. 出たものから、中身を取り出し直して確認する
...
| **全枚が同じ組み方** | 単調になり、どこを注視すればよいか分からなくなる | 3 |
| **枚ごとに列の役割が変わる** | 読み手が毎回、対応づけを覚え直す | 3 |
...
| 6 本人には期待が見える | **After staring at the generating code you tend to see what you expect rather than what rendered, so look at the images fresh** ／ Your first render usually has a few real issues… **and stop** ／ Visual QA の欠陥の並び |
...
| `render-check.md` | 描画して確認する手順である。**書いたものと、描画されたものは違う** |
