...
"""調査対象の原文を取得し、書こうとしているものが原文に在るかを検査する。
  ...
  python3 scripts/cli.py fetch  <URL> [--dir <保存先>]
  python3 scripts/cli.py verify <原文> --as identifier|quote|text <照合するもの>...
                           [--from <1行1個で書いたファイル>] [--near <アンカー>] [--within <行数>]
  python3 scripts/cli.py list   [--dir <保存先>]

**要約を経由して原典を読んではいけない。**
項目名は説明ではなくキーである。1文字違えば、そのキーで参照する実装は必ず空を返す。
...
def listing(outdir: str) -> list[tuple[str, int, str]]:
    """取得したものを並べる。**印字はしない。**"""
...
def lst_lines(outdir: str) -> list[str]:
    """一覧の行。**印字する側と、機械へ返す側が、同じ文字列を使う。**"""
    if not os.path.isdir(outdir):
        return [f"まだ何も取得していない: {outdir}"]
    rows = listing(outdir)
    out = [f"{'取得した日':12}{'行':>8}  出どころ"]
    out += [f"{d:12}{l:>8,}  {u}" for d, l, u in rows]
    out.append(f"── {len(rows)} 件")
    return out


def lst(outdir: str) -> int:
    for line in lst_lines(outdir):
        print(line)
    return 0
...
def report(path: str, s: Scan, near: str | None, within: int) -> int:
    for line in report_lines(path, s, near, within):
        print(line)
    return 1 if (s.missing or s.unreadable) else 0


def report_lines(path: str, s: Scan, near: str | None, within: int) -> list[str]:
    """報告の行。**印字する側と、機械へ返す側が、同じ文字列を使う。**"""
    out: list[str] = []

    def print(*a):          # noqa: A001 ── 行を集めるだけ。名前は元の形を保つ
        out.append(" ".join(str(x) for x in a) if a else "")
