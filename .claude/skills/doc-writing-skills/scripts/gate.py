...
def _synonym(units: list[Unit]) -> list[Finding]:
    """概念7。同じ文脈では、1つの意味に1つの語だけを割り当てる。

    **インラインコードの中は見ない。**そこに在るのは識別子と原文の引用であり、
...
# 和語の述部と、その言い換え先。
# 公用文作成の考え方 Ⅲ－４ ウ「重厚さや正確さを高めるには、述部に漢語を用いる」に拠る。
# 述部を和語にすると意味の範囲が広がり、読み手が何の操作かを特定できない。
WAGO: list[tuple[str, str]] = [
    # ふつうの日本語（引き返す ・ 引き受ける ・ 見落とす ・ 落とし穴 ・ 割り当てる ・
    # 積み上がる ・ 後回し ・ 推測が当たる）は対象にしない。**言い換える先が無い。**
    (r"叩[くきけ]",                                    "実行する"),
    (r"手が伸び",                                      "書き換える（文ごと直す）"),
...
def _wago_predicate(u: Unit) -> list[str]:
    """公用文 Ⅲ－４。述部を和語にすると、意味の範囲が広がって操作が特定できない。"""
    hits = []
    for rx, to in _WAGO:
        m = rx.search(u.text)
        if m:
...
CHECKS: list[Check] = [
    ...
    Check("述部が和語である", "概念8", _wago_predicate,
          ...
          "公用文 Ⅲ－４ ウ。引用は見ない ── 原文を書き換えてはならない"),
