...
def _panel(t: Topic, theme: str, ask: bool = True) -> str:
    ...
    if t.grounds:
        folds.append(_fold(f"なぜそう判定できるか（根拠 {len(t.grounds)}件）",
...
def audit(topics: list["Topic"], extras=None) -> list[str]:
    ...
    **見えないもの**が4つある —— 答えの中身が正しいか、反証が十分か、図が主張を運べているか、
    ...
    out: list[str] = []
    if len(topics) > 5:
        out.append(f"論点が{len(topics)}件ある。3〜5件に絞る ── "
    ...
    for t in topics:
        ...
        if t.status != "決着" and (t.pick or t.decision) and not used.get(t.no):
            out.append(f"論点{t.no}: 答えを持つが、下流のどの論点にも使われていない ── "
                       "試されていないので、承認へ出さない")
    return out
...
def deck(theme: str, topics: list[Topic], intro: str | None = None,
         ...
         queue: list[tuple[int, str]] | None = None) -> str:
    ...
    **queue を渡すと、いま見る論点だけが回答欄を持つ。** 渡すのは (論点の番号, なぜ今か)
    ...
    for line in audit(topics, extras):
    ...
    for i, t in enumerate(topics, start=1):
        ...
        panels.append(f'<div id="p{i}" hidden>{_panel(t, theme, ask=queue is None or t.no in front)}</div>')
