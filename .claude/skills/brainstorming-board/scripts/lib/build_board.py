...
def cell(x: str) -> str:
    """1つの欄に2つのことが入っているものを、**主張と説明の2行**にする。
...
def _panel(t: Topic, theme: str, ask: bool = True, prev: dict | None = None,
    ...
    # 面は別のことなので、器も名前も別になる。
    # **名前はここで確定させる** ── 生成物の文言を後から照合して書き換えると、
...
def audit(topics: list["Topic"], extras=None, queue=None) -> list[str]:
    ...
    見るのは4つ。どれも、実際にやらかしたものである。
      ...
      2 いま見る論点を示しているか    —— 8件を同時に出し、順番の管理を承認する側へ渡した。
                                      **開いている論点が無い回（まとめ）は、示すものが無いので見ない**
      ...
      6 答えに完成イメージが在るか  —— 図も実例も無いまま承認を求めた。人は文章だけで決定を
                                      受け取れない。**待ちの論点は見ない** —— 承認へ出していない
    ...
    **見えないもの**が4つある —— 答えの中身が正しいか、反証が十分か、図が主張を運べているか、
    ...
    for t in topics:
        # **待ちの論点は、承認へ出していない** ── 完成イメージを要求するのは、
        # 答えを承認へ出す回である
        if t.status != "waiting" and t.answer and not (t.figures or t.example):
