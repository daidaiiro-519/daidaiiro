...
**形をコードの中の文字列に散らすと、板ごとに違う形が出る**（実際に、節の1つだけ
別の入れ物で組まれていた）。ここが読むのは `references/board.template.html` 1枚で、
差し込む場所（`{{名前}}`）も、部品の名前も、そのファイルが決める。

    from .template import part
    part("fold", summary="この答えの前提", body=...)
...
TEMPLATE = REFERENCES / "board.template.html"
