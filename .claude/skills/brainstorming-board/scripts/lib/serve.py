...
"""承認の画面を配り、押された回答を1件ずつファイルへ書き出す小さなサーバー。
    ...
    python3 scripts/cli.py serve <配るディレクトリ> [--port 8731] [--answers <蓄積先>]
...
def run(root: str = ".", answers: str = "answers", port: int = 8731) -> int:
    """配って、押された回答を蓄積する。**止められるまで返らない。**"""
    Handler.answers_dir = Path(answers).resolve()
    ...
    with ThreadingHTTPServer(("127.0.0.1", port), handler) as httpd:
        ...
        print(f"開く  : http://127.0.0.1:{port}/")
        try:
            httpd.serve_forever()
...
def main() -> int:
    ...
    a = p.parse_args()
    return run(a.root, a.answers, a.port)
