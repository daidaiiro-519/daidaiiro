...
図の幅は 814 に揃える。幅は中身の大きさから決まるので、合わせる手は
...
#   reroute ── 点線だが、除外ではない。捨てずに別の決めごとへ送る先
...
def sieve(theme: dict, raw: bool = False):
    ...
    nodes = [
        ...
        {"id": "drop2", "label": "除外しない ── 「場を決める」へ送る", "role": "reroute"},
    ]
    edges = [
...
# ── 2枚目：規則文書1本の内訳 ──────────────────────────────────────
# 文書 ＝ メタデータ ＋ 規則の並び。規則1件は3つを抱え、その3つが
# それぞれ何を抱えるかまで見せる。抱える関係は入れ子の図でそのまま描く
# （囲みは辺の相手にならないので、中の1つへつなぎたい形には使わない）。
def doc(theme: dict, raw: bool = False):
    """規則文書1本が何を抱えるか。検証方法の「最後に見直した日」が今回の追加。"""
